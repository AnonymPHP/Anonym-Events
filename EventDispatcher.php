...
class EventDispatcher
{
    ...
    public function fire($eventName = null)
    {
        if (is_string($eventName) || $eventName instanceof EventDispatch) {
            if ($eventName instanceof EventDispatch) {
                ...
                if (strstr($eventName, "\\")) {
                    ...
                    if (isset($this->list[$eventName])) {
                        ...
                        throw new EventNotFoundException(sprintf('%s isimli bir ön tanýmlý event bulunamadý', $eventName));
                    }
                }
            }
            if ($this->hasListiner($eventName)) {
                ...
                throw new EventListenerException(sprintf('%s adýndaki Event\' in herhangi bir dinleyicisi yok', $eventName));
            }
        } else {
            throw new EventException('Girdiðiniz Event, geçerli bir event deðil');
        }
    }
    ...
    private function runListenersHandle(array $listeners = [], $eventName = null)
    {
        ...
        foreach ($listeners as $listener) {
            ...
            if ($listener instanceof EventListener) {
                ...
                throw new EventListenerException(sprintf('%s listener sýnýfý EventListenerInterface\' e sahip olmalýdýr',
                    get_class($listener)));
            }
        }
        return $response;
    ...
    public function getListeners($eventName = '')
    {
        if (!is_string($eventName)) {
            throw new EventNameException('Event adý geçerli bir string deðeri olmalýdýr');
        }
